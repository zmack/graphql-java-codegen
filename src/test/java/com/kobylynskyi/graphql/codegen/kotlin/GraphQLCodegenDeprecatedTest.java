...
class GraphQLCodegenDeprecatedTest {
    ...
    @Test
    void generateSkipsDeprecated() throws Exception {
        mappingConfig.setAddDeprecatedAnnotation(false);
        new KotlinGraphQLCodegen(Collections.singletonList("src/test/resources/schemas/deprecated-with-msg.graphqls"),
                outputBuildDir, mappingConfig, TestUtils.getStaticGeneratedInfo()).generate();
        ...
        for (File file : files) {
            assertSameTrimmedContent(
                    new File(String.format("src/test/resources/expected-classes/kt/deprecated-no-annotation/%s.txt", file.getName())),
                    file);
        }
    }
}
